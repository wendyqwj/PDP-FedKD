...
from models.Update import LocalUpdateClip, LocalUpdate
from models.FedDP import ServerAggDP
...
if __name__ == '__main__':
    ...
    if args.dp_mechanism != 'no_dp':
        clients = [LocalUpdateClip(args=args, dataset=dataset_train, idxs=dict_users[i]) for i in
    ...
    for iter in range(args.epochs):
        ...
        server = ServerAggDP(args=args)
        if args.avg:
            w_glob, bytes_overhead_comm = server.FedAvg(w=copy.deepcopy(w_locals))
        else:
            w_glob, bytes_overhead_comm = server.FedWeightAvg(w=copy.deepcopy(w_locals),
                                                              size=copy.deepcopy(weight_locols))
