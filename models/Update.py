    ...
    def train(self, net):
        ...
        for images, labels in self.ldr_train:
            ...
            for i in range(0, index + 1):
                net.zero_grad()
                start = i * self.args.serial_bsserial_bs
...
class LocalUpdateClip(object):
    ...
    def train(self, net):
        ...
        # print('pre clip w'.format(net.state_dict()))
        if self.args.dp_mechanism != 'no_dp':
            self.clip_grad(net)
        # print('after clip w'.format(net.state_dict()))
        return net.state_dict(), sum(epoch_loss) / len(epoch_loss)

    def clip_grad(self, net):
        if self.args.dp_mechanism == 'Laplace':
            # Laplace use 1 norm
            self.local_gradients_clip(net, self.args.dp_clip, norm=1)
        elif self.args.dp_mechanism == 'Gaussian' or self.args.dp_mechanism == 'MA':
            # Gaussian use 2 norm
            self.local_gradients_clip(net, self.args.dp_clip, norm=2)

    def local_gradients_clip(self, net, clipping, norm):
        grad_samples = [x.grad_sample for x in net.parameters()]
        per_param_norms = [
            g.reshape(len(g), -1).norm(norm, dim=-1) for g in grad_samples
        ]
...
# 本地客户端寻训练，返回本地更新，包括梯度、模型参数和本地损失
class LocalUpdateGrad(object):
    def __init__(self, args, dataset=None, idxs=None):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        self.selected_clients = []
    ...
    def train(self, net):
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                batch_loss.append(loss.item())
            epoch_loss.append(sum(batch_loss) / len(batch_loss))
        # computer gradient update
        client_gradient = [(param.grad.clone(), param.shape) for param in net.parameters()]

        return net.state_dict(), sum(epoch_loss) / len(epoch_loss), client_gradient
