...
class LocalUpdateDP(object):
    ...
    def train(self, net):
        ...
        #for name, params in net.named_parameters():
            #print('-->name: ', name, '-->var: ', torch.var(params.grad))
            #print('-->name:', name, '-->grad_requirs:', params.requires_grad, '-->grad_value:', params.grad)
        return net.state_dict(), loss_client, net.named_parameters()
    ...
    def train(self, net):
        ...
        for images, labels in self.ldr_train:
            ...
            for i in range(0, index + 1):
                ...
                # print("total_grads: %f" % total_grads)
...
class LocalUpdate(object):
    ...
    def train(self, net):
        ...
        for iter in range(self.args.local_ep):
            ...
            time_localep = end - start
            time_local +=time_localep
            epoch_loss.append(sum(batch_loss) / len(batch_loss))
            self.lr = scheduler.get_last_lr()[0]
        time_perep = time_local/self.args.local_ep
        return net.state_dict(), sum(epoch_loss) / len(epoch_loss), net.named_parameters(), time_perep
